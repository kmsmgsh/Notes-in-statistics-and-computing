...
就是离散的的分布嘛，记差了还行
\includegraphics{https://upload.wikimedia.org/wikipedia/commons/thumb/8/85/Discrete_probability_distrib.svg/440px-Discrete_probability_distrib.svg.png}
...
\includegraphics{Notes-in-statistics-and-computing_files/figure-latex/unnamed-chunk-4-1.pdf}
...
\includegraphics{Notes-in-statistics-and-computing_files/figure-latex/unnamed-chunk-5-1.pdf}
...
\includegraphics{http://www.sthda.com/sthda/RDoc/figure/graphs/r-plot-pch-symbols-points-in-r.png}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x <-}\StringTok{ }\NormalTok{out}\OperatorTok{$}\NormalTok{x }
\NormalTok{y <-}\StringTok{ }\NormalTok{out}\OperatorTok{$}\NormalTok{y}
\NormalTok{lupost <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(beta, x, y, ...) \{ }
\NormalTok{  eta <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(x }\OperatorTok{%*%}\StringTok{ }\NormalTok{beta)}
\NormalTok{  logp <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(eta }\OperatorTok{<}\StringTok{ }\DecValTok{0}\NormalTok{, eta }\OperatorTok{-}\StringTok{ }\KeywordTok{log1p}\NormalTok{(}\KeywordTok{exp}\NormalTok{(eta)),}\OperatorTok{-}\StringTok{ }\KeywordTok{log1p}\NormalTok{ (}\KeywordTok{exp}\NormalTok{(}\OperatorTok{-}\StringTok{ }\NormalTok{eta))) }
\NormalTok{  logq <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(eta }\OperatorTok{<}\StringTok{ }\DecValTok{0}\NormalTok{, }\OperatorTok{-}\StringTok{ }\KeywordTok{log1p}\NormalTok{(}\KeywordTok{exp}\NormalTok{(eta)), }\OperatorTok{-}\StringTok{ }\NormalTok{eta }\OperatorTok{-}\StringTok{ }\KeywordTok{log1p}\NormalTok{ (}\KeywordTok{exp}\NormalTok{(}\OperatorTok{-}\StringTok{ }\NormalTok{eta))) }
\NormalTok{  logl <-}\StringTok{ }\KeywordTok{sum}\NormalTok{(logp[y }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{]) }\OperatorTok{+}\StringTok{ }\KeywordTok{sum}\NormalTok{(logq[y }\OperatorTok{==}\StringTok{ }\DecValTok{0}\NormalTok{]) }
  \KeywordTok{return}\NormalTok{(logl }\OperatorTok{-}\StringTok{ }\KeywordTok{sum}\NormalTok{(beta}\OperatorTok{^}\DecValTok{2}\NormalTok{) }\OperatorTok{/}\StringTok{ }\DecValTok{8}\NormalTok{)}
\NormalTok{\}}
\NormalTok{beta.init <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{coefficients}\NormalTok{(out))}
\NormalTok{out <-}\StringTok{ }\KeywordTok{metrop}\NormalTok{(lupost, beta.init, }\FloatTok{1e3}\NormalTok{, }\DataTypeTok{x =}\NormalTok{ x, }\DataTypeTok{y =}\NormalTok{ y)}
\NormalTok{out2=}\KeywordTok{metrop}\NormalTok{(out,}\DataTypeTok{x=}\NormalTok{x,}\DataTypeTok{y=}\NormalTok{y)}
\end{Highlighting}
\end{Shaded}
...
这个很清晰，第一块是\(\boldsymbol y\) 的Normal，第二块是\(\sigma^2\) 的inverse-gamma，第三块是\(\beta_j\) 的Normal hierachical prior with 超参数\(\tau_j\) 服从\$f(\tau)=\frac{\lambda^{2}}{2} e\textsuperscript{\{-\lambda}\{2\} \tau\_\{j\}\^{}\{2\} / 2\} \$.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{df=}\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x=}\KeywordTok{rep}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{,}\DataTypeTok{each=}\DecValTok{2}\NormalTok{),}\DataTypeTok{y=}\DecValTok{6}\OperatorTok{:}\DecValTok{1}\NormalTok{,}\DataTypeTok{z=}\NormalTok{letters[}\DecValTok{1}\OperatorTok{:}\DecValTok{6}\NormalTok{])}
\NormalTok{df2=df[}\KeywordTok{sample}\NormalTok{(}\KeywordTok{nrow}\NormalTok{(df)),}\DecValTok{3}\OperatorTok{:}\DecValTok{1}\NormalTok{]}
...
\NormalTok{rating <-}\StringTok{ }\NormalTok{lego_movie }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{html_nodes}\NormalTok{(}\StringTok{"strong span"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# 这个"strong span"是指什么意思？}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{PostMentionUser=}\KeywordTok{html_nodes}\NormalTok{(a,}\StringTok{".PostMention"}\NormalTok{)}\OperatorTok{%>%}\StringTok{ }\KeywordTok{html_text}\NormalTok{()}
\KeywordTok{head}\NormalTok{(PostMentionUser)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{html_nodes}\NormalTok{(a,}\StringTok{".Post-body"}\NormalTok{)}\OperatorTok{%>%}
\StringTok{  }\KeywordTok{html_text}\NormalTok{()->body}
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{2300}\OperatorTok{%%}\DecValTok{100} 
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{230}\OperatorTok{%%}\DecValTok{100}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(tcltk2)}

\NormalTok{u <-}\StringTok{ }\DecValTok{1}\OperatorTok{:}\DecValTok{2000}

\KeywordTok{plot.new}\NormalTok{()}

\NormalTok{pb <-}\StringTok{ }\KeywordTok{tkProgressBar}\NormalTok{(}\StringTok{"进度"}\NormalTok{,}\StringTok{"已完成 %"}\NormalTok{,  }\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{)}

\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in}\NormalTok{ u) \{}

\NormalTok{x<-}\KeywordTok{rnorm}\NormalTok{(u)}

\KeywordTok{points}\NormalTok{(x,x}\OperatorTok{^}\DecValTok{2}\NormalTok{,}\DataTypeTok{col=}\NormalTok{i)}

\NormalTok{info <-}\StringTok{ }\KeywordTok{sprintf}\NormalTok{(}\StringTok{"已完成 %d%%"}\NormalTok{, }\KeywordTok{round}\NormalTok{(i}\OperatorTok{*}\DecValTok{100}\OperatorTok{/}\KeywordTok{length}\NormalTok{(u)))}

\KeywordTok{setTkProgressBar}\NormalTok{(pb, i}\OperatorTok{*}\DecValTok{100}\OperatorTok{/}\KeywordTok{length}\NormalTok{(u), }\KeywordTok{sprintf}\NormalTok{(}\StringTok{"进度 (%s)"}\NormalTok{, info), info)}
...
\tightlist
\item
  \href{https://github.com/r-lib/progress}{进度条}
  我觉得这个更好用
\end{itemize}
...
\NormalTok{pb <-}\StringTok{ }\NormalTok{progress_bar}\OperatorTok{$}\KeywordTok{new}\NormalTok{(}\DataTypeTok{total =} \DecValTok{100}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{100}\NormalTok{) \{}
\NormalTok{  pb}\OperatorTok{$}\KeywordTok{tick}\NormalTok{()}
  \KeywordTok{Sys.sleep}\NormalTok{(}\DecValTok{1} \OperatorTok{/}\StringTok{ }\DecValTok{100}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
